...
- ``ScriptedPg`` + ``make_pg_ctx`` — a psycopg-shaped seam (pool -> connection ->
  cursor / transaction) whose cursor records executed SQL and replays programmed
  fetch results, so ``stores.py`` runs its real SQL-issuing control flow without a
  live database (real SQL correctness is proven by the ecosystem e2e leg).
- ``FakeRedis`` + ``make_redis_ctx`` — the string/counter operations the rate
  limiter and the shared bootstrap token use.
- In-memory ``FakeUsersStore`` / ``FakeSessionsStore`` / ``FakeInvitesStore`` +
...
class FakeUniqueViolation(psycopg.errors.UniqueViolation):
    """A ``UniqueViolation`` whose ``.diag.constraint_name`` is programmable, so a
    store test can drive the email-taken / id-collision split without a live DB."""
...
class _FakeAdminGuard:
    """Mirrors the real ``_AdminGuard``: the last-admin count and its mutation over
    the same in-memory rows, as one 'transaction' the guard hook can race."""
...
class FakeUsersStore:
    def __init__(self) -> None:
        ...
        # Fires once when a guarded txn "acquires the lock", so a test can simulate
        # a concurrent guarded removal committing before this one re-counts.
...
def build_request(
    ...
    *,
    ...
    """A Starlette ``Request`` carrying a JSON body, a client peer, headers, and
