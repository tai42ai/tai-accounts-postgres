...
async def test_combined_demote_and_reenable_applies_both(wire):
    # A currently-DISABLED admin, demoted AND re-enabled in one request. The role
    # change routes it through the guarded branch; the re-enable must NOT be dropped.
    _add_user(wire, "usr-1", role="admin")  # the surviving admin
...
async def test_concurrent_admin_removals_cannot_both_orphan(wire):
    ...
    # While this disable of usr-1 holds the guard lock, a concurrent request has
    # already disabled the other admin; the under-lock count then sees zero others,
    # so this one refuses — mirroring the bootstrap owner-race guard.
...
async def test_demote_reevaluates_disabled_under_lock(wire):
    # The pre-lock snapshot shows usr-1 DISABLED (so snapshot-based code would skip
    # the orphan count and demote freely); under the lock a concurrent request has
    # re-enabled usr-1 and disabled usr-2, so the demote must re-read the enabled
    # state, count zero other enabled admins, and refuse.
...
async def test_update_target_vanishes_under_lock_404(wire):
    # The pre-lock snapshot found the row, but a concurrent delete removed it before
    # the lock was acquired; the under-lock re-read returns None, so the mutation
    # 404s rather than acting on a ghost.
    _add_user(wire, "usr-1", role="admin")
    _add_user(wire, "usr-2", role="viewer")
