...
All SQL lives here; service and route code never write SQL. Each store opens the
kit ``PostgresClient`` pool from the plugin's ``AccountsPgSettings`` and speaks in
plain rows. Unique violations are split by CONSTRAINT NAME — never a generic
catch — so an email conflict and a ``user_id`` collision take different paths.
Expired/consumed rows are swept opportunistically on the write that would create
new ones; the sweep has no private error handling, so a sweep failure propagates
with the enclosing operation (a quietly failing sweep is a silent-degrade path).
"""
...
# Fixed advisory-lock key serializing the account-count-then-mutate operations
# that must not interleave: the bootstrap zero-users check + owner insert, and the
# last-admin guard's count-then-disable/demote/delete. A stable constant derived
# from the table name (ASCII "ACCOUS"), well within Postgres' signed 64-bit
# advisory-lock key range.
...
class UsersStore:
    ...
    async def create(self, user_id: str, email: str, role: str, password_hash: str | None = None) -> str:
        ...
        An ``accounts_users_email_unique`` violation raises :class:`EmailTakenError`.
        An ``accounts_users_user_id_unique`` violation regenerates the id and
        retries, at most ``_MAX_ID_ATTEMPTS`` times, then raises naming the
        constraint — an email conflict never enters the regeneration loop, and the
    ...
    async def create_owner_if_first(
        ...
        One transaction: take the fixed advisory xact lock (serializing concurrent
        bootstraps), count users, and insert only when the count is zero. The lock
        releases at commit. Returns the inserted row, or ``None`` when users
        already exist (someone won the race / a prior owner exists).
        """
        async with (
            client_ctx(PostgresClient, self._settings) as pool,
            pool.connection() as conn,
    ...
    async def count_other_enabled_admins(self, user_id: str) -> int:
        ...
        leave the deployment with no enabled admin, so the caller 409s.
        """
        async with (
            client_ctx(PostgresClient, self._settings) as pool,
            pool.connection() as conn,
    ...
    async def admin_guard_txn(self) -> AsyncIterator[_AdminGuard]:
        ...
        The lock is the SAME key the bootstrap owner insert takes, so a concurrent
        guarded removal blocks at the lock and re-evaluates the admin count against
        committed state once it acquires — two removals of the last two admins can
        never both pass. On the yielded guard the caller RE-READS the target's
        role/disabled, counts other enabled admins, and applies its mutation (row +
        any credential cleanup) — all on this one cursor; the re-read, the count, and
        the mutation commit (or roll back) together when the block exits, closing the
...
class _AdminGuard:
    """The last-admin re-read, count, and the mutation it authorizes, bound to one
    advisory-locked cursor so they are a single serialized transaction.

    Every method runs on the guard's own cursor — the users row, its sessions, and
    its invites all live in the same database, so the credential cleanup a guarded
    disable/delete performs runs in THIS transaction on THIS connection, never a
    second pool checkout taken while the lock is held.
    """
    ...
    async def read_target(self, user_id: str) -> dict[str, Any] | None:
        ...
        authoritative state the orphan check and the guarded-vs-unguarded branch
        decide on, never the caller's pre-lock snapshot (which a concurrent
        re-enable/demote could have made stale)."""
    ...
    async def delete_sessions_for_user(self, user_id: str) -> None:
        """Revoke the user's sessions on the guard's connection — the same
        transaction as the row mutation, so no nested pool checkout under the lock."""
...
class SessionsStore:
    ...
    async def create(self, token_hash: str, user_id: str, absolute_expires_at: Any) -> None:
        async with (
            ...
            # Opportunistic sweep of absolute-expired rows, piggybacked on mint —
            # no private error handling, so a sweep failure fails the mint loudly.
...
class InvitesStore:
    ...
    async def consume(self, token_hash: str, now: Any) -> str | None:
        ...
        Single-use and TTL are enforced by the row update, not a read-then-write:
        the ``consumed_at IS NULL AND expires_at > now`` predicate makes a replay,
        an expired token, and an unknown token all return no row.
        """
        async with (
            client_ctx(PostgresClient, self._settings) as pool,
            pool.connection() as conn,
